...
python conditional_copy_paste.py \
    --work_dir ".\example" \
    --config_filename "plasticbag_v1.yaml" \
    --do_feather \
    --do_edge_blur \
...
from utils import polygon_to_bbox, bbox_to_yolo, bbox_to_yolo_segmentation
from conditional_rule_solver import choose_position_scale_angle_depend_on_rule
...
def process_one_img(
    bg_img_name, 
    background_img_folder, # 偏变量
    object_dict, 
    number_rules, 
    object_img_folder, 
    data_creation_rules, 
    background_annotations_folder,
    do_feather,
    do_edge_blur,
    do_brightness_scale,
    feather_radius,
    output_img_dir,
    output_ann_dir,
    output_yolo_bbox_ann_dir,
    output_yolo_seg_ann_dir,
):
        ...
        for cls_id, (cls_name, png_list) in enumerate(object_dict.items()):
            ...
            for obj_img_name in chosen_pngs:
                ...
                if data_creation_rules:  # TODO 根据规则约束调整贴图位置和大小
                    ...
                    position_rule, size_rule, angle_rule, add_this_obj = choose_position_scale_angle_depend_on_rule(
                        cls_name, 
                        data_creation_rules, 
        ...
        out_img_name = f"synthetic_{bg_img_name}"
...
def main(
    ...
    partial_process_one_img = partial(
        process_one_img,
        background_img_folder=background_img_folder, # 偏变量
        object_dict=object_dict, 
        number_rules=number_rules, 
        object_img_folder=object_img_folder, 
        data_creation_rules=data_creation_rules, 
        background_annotations_folder=background_annotations_folder,
        do_feather=do_feather,
        do_edge_blur=do_edge_blur,
        do_brightness_scale=do_brightness_scale,
        ...
        output_yolo_bbox_ann_dir=output_yolo_bbox_ann_dir,
        output_yolo_seg_ann_dir=output_yolo_seg_ann_dir,
    )
    with Pool(processes=4) as pool:  
        # results = pool.map(process_one_img, background_list)
        results = list(tqdm(pool.imap(partial_process_one_img, background_list), total=len(background_list)))
